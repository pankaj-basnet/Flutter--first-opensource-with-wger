        ...
        children: [
          ...
          ListTile(
            ...
            leading: CircleAvatar(
              backgroundColor:
                  _RF.primaryContainer(context),
              child: Icon(Icons.restaurant_menu,
                  color: _RF.primary(context), size: 20),
            ),
            title: Text(mealName,
                style: _RF.text(context).titleMedium?.copyWith(
                      fontWeight: FontWeight.bold,
                    )),
            subtitle: Text(
              '${mealTime.isNotEmpty ? '$mealTime  ·  ' : ''}${_totalKcal.toStringAsFixed(0)} kcal',
              style: _RF.text(context).bodySmall?.copyWith(
          ...
          if (_expanded) ...[
            ...
            if (_items.isEmpty)
              Padding(
                ...
                child: Text('No ingredients defined',
                    style: _RF.text(context).bodySmall?.copyWith(
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  NutritionChipRow(
                    kcal: _totalKcal,
                    protein: _items.fold(0.0,
                        (s, i) =>
                            s + ((i['protein'] as num?)?.toDouble() ?? 0)),
                    carbs: _items.fold(0.0,
                        (s, i) =>
                            s +
                            ((i['carbohydrates'] as num?)?.toDouble() ?? 0)),
                    fat: _items.fold(0.0,
                        (s, i) =>
                            s + ((i['fat'] as num?)?.toDouble() ?? 0)),
                  ),
                ],
              ),
            ),
...
class _IngredientRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final name = item['ingredient_name'] as String? ?? item['name'] as String? ?? '…';
    ...
    return ListTile(
      ...
      trailing: Text(
        '$kcal kcal',
        style: _RF.text(context).labelSmall?.copyWith(
              color: _RF.primary(context),
